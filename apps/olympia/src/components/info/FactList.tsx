...
export function FactList({ section }: FactListProps) {
  return (
    <section aria-labelledby={`section-${section.id}`} className="mb-8">
      <h2 id={`section-${section.id}`} className="text-lg font-bold text-neutral-900 mb-2">
        {section.headline}
      </h2>
      {section.intro && <p className="text-sm text-neutral-500 mb-4">{section.intro}</p>}
        ...
        {section.facts.map((fact) => (
          <div key={fact.id} className="px-4 py-3 bg-white">
            ...
            <dd className="text-base text-neutral-900 leading-snug">
              {fact.value}
              {fact.sourceRef && (
                <span className="ml-2 text-xs text-neutral-400">(Quelle: {fact.sourceRef})</span>
              )}
            </dd>
          </div>
        ))}
